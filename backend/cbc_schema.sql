-- ================================================================
-- Kadem & Zetu School Management System — Production Schema v5.0
-- Kenya KICD Competency Based Curriculum
-- Multi-tenant · Roles: SUPER_ADMIN, PRINCIPAL, DEPUTY_PRINCIPAL,
--   HOD, TEACHER, BURSAR
-- Consolidated — safe to run on a fresh Neon database.
-- All CREATE TABLE / ALTER TABLE use IF NOT EXISTS / IF NOT EXISTS
...
-- ================================================================
-- TEACHERS  (profile, linked to user account)
-- ================================================================
...
CREATE TABLE IF NOT EXISTS payment_plans (
  ...
  ai_daily_limit   INTEGER       DEFAULT 50,
  is_active        BOOLEAN       DEFAULT TRUE,
  created_at       TIMESTAMPTZ   DEFAULT NOW(),
  updated_at       TIMESTAMPTZ   DEFAULT NOW()
);
...
-- Deferred FK (last_payment_id → subscription_payments)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS report_cards (
  ...
  -- Core Competencies (1=Not Observed … 4=Exceptional)
  cc_communication      SMALLINT CHECK (cc_communication      BETWEEN 1 AND 4),
  cc_critical_thinking  SMALLINT CHECK (cc_critical_thinking  BETWEEN 1 AND 4),
  cc_creativity         SMALLINT CHECK (cc_creativity         BETWEEN 1 AND 4),
  cc_citizenship        SMALLINT CHECK (cc_citizenship        BETWEEN 1 AND 4),
  cc_digital_literacy   SMALLINT CHECK (cc_digital_literacy   BETWEEN 1 AND 4),
  cc_learning_to_learn  SMALLINT CHECK (cc_learning_to_learn  BETWEEN 1 AND 4),
  cc_self_efficacy      SMALLINT CHECK (cc_self_efficacy      BETWEEN 1 AND 4),
  -- Values (1=Needs Improvement … 4=Excellent)
  val_respect           SMALLINT CHECK (val_respect           BETWEEN 1 AND 4),
  val_responsibility    SMALLINT CHECK (val_responsibility    BETWEEN 1 AND 4),
  val_integrity         SMALLINT CHECK (val_integrity         BETWEEN 1 AND 4),
  val_unity             SMALLINT CHECK (val_unity             BETWEEN 1 AND 4),
  val_peace             SMALLINT CHECK (val_peace             BETWEEN 1 AND 4),
  val_patriotism        SMALLINT CHECK (val_patriotism        BETWEEN 1 AND 4),
  val_social_justice    SMALLINT CHECK (val_social_justice    BETWEEN 1 AND 4),
...
CREATE INDEX IF NOT EXISTS idx_students_fullname          ON students(school_id, LOWER(first_name || ' ' || last_name));
CREATE INDEX IF NOT EXISTS idx_students_firstname_lower   ON students(school_id, LOWER(first_name) text_pattern_ops);
CREATE INDEX IF NOT EXISTS idx_students_lastname_lower    ON students(school_id, LOWER(last_name) text_pattern_ops);
CREATE INDEX IF NOT EXISTS idx_students_admission_lower   ON students(school_id, LOWER(admission_number) text_pattern_ops);
CREATE INDEX IF NOT EXISTS idx_teachers_firstname_lower   ON teachers(school_id, LOWER(first_name) text_pattern_ops);
CREATE INDEX IF NOT EXISTS idx_teachers_lastname_lower    ON teachers(school_id, LOWER(last_name) text_pattern_ops);
CREATE INDEX IF NOT EXISTS idx_teachers_tsc_lower         ON teachers(school_id, LOWER(tsc_number) text_pattern_ops);
...
INSERT INTO learning_areas (code, name, stage, department, is_core, sort_order, kicd_ref, pathway) VALUES
  -- ECDE
  ('ECDE-LA',      'Language Activities',                  'ECDE',             'Languages',      TRUE,  1,  'KICD/CBC/ECDE/001', NULL),
  ('ECDE-MA',      'Mathematical Activities',              'ECDE',             'Mathematics',    TRUE,  2,  'KICD/CBC/ECDE/002', NULL),
  ('ECDE-ENV',     'Environmental Activities',             'ECDE',             'Science',        TRUE,  3,  'KICD/CBC/ECDE/003', NULL),
  ('ECDE-PSY',     'Psychomotor & Creative Arts',          'ECDE',             'Arts',           TRUE,  4,  'KICD/CBC/ECDE/004', NULL),
  ('ECDE-RE',      'Religious Education',                  'ECDE',             'Social Studies', FALSE, 5,  'KICD/CBC/ECDE/005', NULL),
  -- Lower Primary (Grade 1–3)
  ('LP-LE',        'Literacy (English)',                   'Lower Primary',    'Languages',      TRUE,  6,  'KICD/CBC/LP/001',   NULL),
  ('LP-LS',        'Literacy (Kiswahili)',                 'Lower Primary',    'Languages',      TRUE,  7,  'KICD/CBC/LP/002',   NULL),
  ('LP-MA',        'Mathematical Activities',              'Lower Primary',    'Mathematics',    TRUE,  8,  'KICD/CBC/LP/003',   NULL),
  ('LP-ENV',       'Environmental Activities',             'Lower Primary',    'Science',        TRUE,  9,  'KICD/CBC/LP/004',   NULL),
  ('LP-CA',        'Creative Arts',                        'Lower Primary',    'Arts',           TRUE,  10, 'KICD/CBC/LP/005',   NULL),
  ('LP-PHE',       'Physical & Health Education',          'Lower Primary',    'Physical Ed',    TRUE,  11, 'KICD/CBC/LP/006',   NULL),
  ('LP-RE',        'Religious Education',                  'Lower Primary',    'Social Studies', FALSE, 12, 'KICD/CBC/LP/007',   NULL),
  -- Upper Primary (Grade 4–6)
  ('UP-EN',        'English',                              'Upper Primary',    'Languages',      TRUE,  13, 'KICD/CBC/UP/001',   NULL),
  ('UP-KIS',       'Kiswahili',                            'Upper Primary',    'Languages',      TRUE,  14, 'KICD/CBC/UP/002',   NULL),
  ('UP-MA',        'Mathematics',                          'Upper Primary',    'Mathematics',    TRUE,  15, 'KICD/CBC/UP/003',   NULL),
  ('UP-SCI',       'Science & Technology',                 'Upper Primary',    'Science',        TRUE,  16, 'KICD/CBC/UP/004',   NULL),
  ('UP-SS',        'Social Studies',                       'Upper Primary',    'Social Studies', TRUE,  17, 'KICD/CBC/UP/005',   NULL),
  ('UP-RE',        'Religious Education',                  'Upper Primary',    'Social Studies', FALSE, 18, 'KICD/CBC/UP/006',   NULL),
  ('UP-CAS',       'Creative Arts & Sports',               'Upper Primary',    'Arts',           TRUE,  19, 'KICD/CBC/UP/007',   NULL),
  ('UP-LS',        'Life Skills',                          'Upper Primary',    'Social Studies', TRUE,  20, 'KICD/CBC/UP/008',   NULL),
  ('UP-AG',        'Agriculture',                          'Upper Primary',    'Science',        FALSE, 21, 'KICD/CBC/UP/009',   NULL),
  -- Junior Secondary (Grade 7–9)
  ('JS-EN',        'English',                              'Junior Secondary', 'Languages',      TRUE,  22, 'KICD/CBC/JS/001',   NULL),
  ('JS-KIS',       'Kiswahili & Kenya Sign Language',      'Junior Secondary', 'Languages',      TRUE,  23, 'KICD/CBC/JS/002',   NULL),
  ('JS-MA',        'Mathematics',                          'Junior Secondary', 'Mathematics',    TRUE,  24, 'KICD/CBC/JS/003',   NULL),
  ('JS-INT',       'Integrated Science',                   'Junior Secondary', 'Science',        TRUE,  25, 'KICD/CBC/JS/004',   NULL),
  ('JS-HC',        'Health Education',                     'Junior Secondary', 'Science',        TRUE,  26, 'KICD/CBC/JS/005',   NULL),
  ('JS-PRE',       'Pre-Technical Studies',                'Junior Secondary', 'Technical',      TRUE,  27, 'KICD/CBC/JS/006',   NULL),
  ('JS-SS',        'Social Studies',                       'Junior Secondary', 'Social Studies', TRUE,  28, 'KICD/CBC/JS/007',   NULL),
  ('JS-RE',        'Religious Education',                  'Junior Secondary', 'Social Studies', FALSE, 29, 'KICD/CBC/JS/008',   NULL),
  ('JS-BS',        'Business Studies',                     'Junior Secondary', 'Social Studies', FALSE, 30, 'KICD/CBC/JS/009',   NULL),
  ('JS-AG',        'Agriculture',                          'Junior Secondary', 'Science',        FALSE, 31, 'KICD/CBC/JS/010',   NULL),
  ('JS-CA',        'Creative Arts',                        'Junior Secondary', 'Arts',           FALSE, 32, 'KICD/CBC/JS/011',   NULL),
  ('JS-PHE',       'Physical & Health Education',          'Junior Secondary', 'Physical Ed',    TRUE,  33, 'KICD/CBC/JS/012',   NULL),
  ('JS-ICT',       'Computer Science',                     'Junior Secondary', 'Technical',      TRUE,  34, 'KICD/CBC/JS/013',   NULL),
  -- Senior Secondary — core (all pathways)
...
-- ================================================================
INSERT INTO payment_plans (name, amount, currency, billing_interval, student_limit, ai_enabled, ai_daily_limit, is_active)
VALUES
  ('Starter',    2500,  'KES', 'term',  150,  TRUE,  30,  TRUE),
  ('Growth',     4500,  'KES', 'term',  500,  TRUE,  100, TRUE),
  ('Pro',        7500,  'KES', 'term',  NULL, TRUE,  300, TRUE),
  ('Enterprise', 15000, 'KES', 'term',  NULL, TRUE,  NULL,TRUE)
