...
router.get("/", auth, roleM(READ), async (req, res) => {
  try {
    ...
    let q = `SELECT ta.*, CONCAT(t.first_name,' ',t.last_name) AS teacher_name, t.tsc_number,
                    CONCAT(c.grade, COALESCE(' '||c.stream,'')) AS class_label, c.grade, c.stream, c.stage,
...
router.post("/", auth, roleM(MANAGE),
  [
    body("teacher_id").notEmpty().isUUID(),
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Verify teacher and class belong to same school — prevents cross-tenant linking
      const [tRes, cRes] = await Promise.all([
        db.query("SELECT school_id FROM teachers WHERE id=$1", [req.body.teacher_id]),
        db.query("SELECT school_id FROM classes WHERE id=$1",  [req.body.class_id]),
      ]);
      if (!tRes.rows.length) return res.status(404).json({ success: false, message: "Teacher not found." });
      if (!cRes.rows.length) return res.status(404).json({ success: false, message: "Class not found." });
      if (req.user.role !== "SUPER_ADMIN" && (tRes.rows[0].school_id !== sid || cRes.rows[0].school_id !== sid))
        return res.status(403).json({ success: false, message: "Teacher or class does not belong to your school." });

      const { teacher_id, class_id, learning_area_id, role, academic_year } = req.body;
      const { rows } = await db.query(
        "INSERT INTO teacher_assignments (school_id,teacher_id,class_id,learning_area_id,role,academic_year) VALUES ($1,$2,$3,$4,$5,$6) RETURNING *",
        [sid, teacher_id, class_id, learning_area_id, role, academic_year]
      );
