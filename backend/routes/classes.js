...
router.get("/", authMiddleware, async (req, res) => {
  try {
    ...
    let q = `SELECT c.*,
             ...
             WHERE 1=1`;
    const p = [];
    if (schoolId) { p.push(schoolId); q += ` AND c.school_id=$${p.length}`; }
  ...
  try {
    const { rows } = await db.query(
      `SELECT c.*, CONCAT(t.first_name,' ',t.last_name) AS class_teacher_name
       FROM classes c LEFT JOIN teachers t ON t.id=c.class_teacher_id WHERE c.id=$1`,[req.params.id]
    );
...
router.post("/", authMiddleware, roleMiddleware(MANAGE),
  [
    ...
    body("academic_year").matches(/^\d{4}$/).withMessage("Invalid academic year"),
  ],
  async (req, res) => {
    ...
    try {
      ...
      const { rows } = await db.query(
        `INSERT INTO classes (school_id, grade, stream, stage, academic_year, class_teacher_id, capacity)
         VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
        [schoolId, grade, stream||null, stage, academic_year, class_teacher_id||null, capacity||40]
      );
...
router.put("/:id", authMiddleware, roleMiddleware(MANAGE), async (req, res) => {
  try {
    ...
    const { rows } = await db.query(
      `UPDATE classes SET stream=COALESCE($1,stream), class_teacher_id=COALESCE($2,class_teacher_id),
...
router.delete("/:id", authMiddleware, roleMiddleware(MANAGE), async (req, res) => {
  try {
    ...
    if (parseInt(studs[0].count) > 0) return res.status(409).json({ success:false, message:"Cannot delete class with active students." });
...
router.get("/:id/students", authMiddleware, async (req, res) => {
  try {
    ...
    if (req.user.role !== "SUPER_ADMIN" && cls[0].school_id !== req.user.school_id) return res.status(403).json({ success:false, message:"Access denied." });
    const { rows } = await db.query(
